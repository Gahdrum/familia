...
import { Plus, Filter, Trash2, Pencil, AlertCircle } from 'lucide-react';
...
const JointWalletPage = () => {
  ...
  })).filter(item => item.total > 0), [categories, transactions]);
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Carteira Conjunta - FinançaFamília</title>
        <meta name="description" content="Gerencie as despesas compartilhadas da família" />
      </Helmet>
      <Header />
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="space-y-6">
            ...
            <Card className="md:col-span-3 shadow-lg">
              ...
              <CardContent>
                {categoryBreakdown.length === 0 ? (
                  <div className="text-sm text-muted-foreground">Nenhuma despesa conjunta cadastrada ainda.</div>
                ) : (
                  <div className="grid grid-cols-2 gap-3">
                    {categoryBreakdown.map(item => (
                      <div key={item.category} className="flex justify-between items-center p-2 rounded-lg bg-muted/50">
          ...
          <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Editar transação conjunta</DialogTitle>
              </DialogHeader>
...
export default JointWalletPage;
